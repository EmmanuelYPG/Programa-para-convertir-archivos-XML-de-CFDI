...
def parsear_xml_cfdi(ruta_xml):
    ...
    try:
        ...
        # OJO: el IEPS (impuesto 003) puede venir en MUCHAS tasas/cuotas distintas,
        # no solo 8% o 0% (p. ej. 3%, 6%, 7%, 9%, 26.5%, 30%, 53%, 160%, o cuotas
        # fijas por unidad en combustibles). Si solo se filtraran 0.08 y 0.00 se
        # perderían esos importes y el Total no cuadraría con la fórmula del CFDI:
        #   (SubTotal - Descuento) + Traslados - Retenciones = Total
        # Por eso cualquier IEPS con otra tasa se acumula en "IEPS Trasladado (otros)".
        iva16 = iva8 = iva0 = iva_exento = 0.0
        ieps8 = ieps0 = ieps_otros = 0.0
        ...
        if impuestos_globales is not None:
            for traslado in impuestos_globales.findall(f'.//{{{NAMESPACES[ns]}}}Traslado'):
                ...
                if impuesto == '002':
                    ...
                    else:                ieps_otros += importe  # cualquier otra tasa/cuota de IEPS
        ...
        registro['IEPS Trasladado (otros)'] = f"{ieps_otros:.2f}"
        ...
        # El ISH (Impuesto Sobre Hospedaje) y otros impuestos locales viven en
        # implocal:ImpuestosLocales DENTRO del Complemento, NO en cfdi:Impuestos.
        # Forman parte del Total del comprobante, así que sin leerlos la fórmula
        #   (SubTotal - Descuento) + Traslados - Retenciones = Total
        # no cuadra cuando la factura trae ISH (típico en hoteles/hospedaje).
        local_ret = local_tras = 0.0
        impl_ns = NAMESPACES['implocal']
        impuestos_locales = xml_root.find(f'.//{{{impl_ns}}}ImpuestosLocales')
        if impuestos_locales is not None:
            for tl in impuestos_locales.findall(f'{{{impl_ns}}}TrasladosLocales'):
                local_tras += float(tl.get('Importe', '0') or '0')
            for rl in impuestos_locales.findall(f'{{{impl_ns}}}RetencionesLocales'):
                local_ret += float(rl.get('Importe', '0') or '0')
        registro['Local retenido']         = f"{local_ret:.2f}"
        registro['Local trasladado (ISH)'] = f"{local_tras:.2f}"
...
HEADERS = [
    ...
    'IEPS Trasladado 8%', 'IEPS Trasladado 0%', 'IEPS Trasladado (otros)', 'IEPS Retenido',
    'Local retenido', 'Local trasladado (ISH)',
    'TotalCombustibles', 'Total'
]


# Columnas cuyo contenido es numérico (importes, tasas, etc.). Se escriben como
# NÚMERO real en Excel —no como texto— para que se puedan sumar/operar y para que
# no aparezca el aviso de "número almacenado como texto".
NUMERIC_HEADERS = {
    'SubTotal', 'Descuento', 'Total',
    'IVA Trasladado 16%', 'IVA Trasladado 8%', 'IVA Trasladado 0%',
    'IVA Exento', 'IVA Retenido', 'ISR Retenido',
    'IEPS Trasladado 8%', 'IEPS Trasladado 0%', 'IEPS Trasladado (otros)',
    'IEPS Retenido',
    'Local retenido', 'Local trasladado (ISH)',
    'SubTotalCombustibles', 'TotalCombustibles',
    'Monto pago', 'Monto total pagos',
    'Imp saldo anterior', 'Imp pagado', 'Imp saldo insoluto',
    'Tipo de cambio',
}


def _a_numero(valor):
    """Convierte un valor a float para escribirlo como número en Excel.
    Devuelve None (celda en blanco) si está vacío, o el valor original
    sin tocar si no es convertible a número."""
        ...
        if registro:
            ...
            for col, header in enumerate(headers, start=1):
                ...
                if header in NUMERIC_HEADERS:
                    ...
                    celda.value = numero
                    # Solo formateamos como número cuando realmente lo es; así las
                    # celdas vacías quedan en blanco y la suma no se rompe.
                    if isinstance(numero, float):
                        celda.number_format = '0.0000' if header == 'Tipo de cambio' else '0.00'
                else:
                    celda.value = valor
            exitosos += 1
        else:
